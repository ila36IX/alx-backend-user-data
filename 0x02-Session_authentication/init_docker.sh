...
if ! docker ps --quiet --filter=name="$CONTAINER_NAME" | grep -q "$CONTAINER_NAME"; then
    ...
    # docker run -d -p 5000:5000 -v "$(pwd):/app" --env AUTH_TYPE=session_auth --env SESSION_DURATION=15 --env  SESSION_NAME=_my_session_id --name "$CONTAINER_NAME" "$IMAGE_NAME" || exit 1
    docker run -d -p 5000:5000 -v "$(pwd):/app" --env AUTH_TYPE=1 --env SESSION_DURATION=15 --env  SESSION_NAME=_my_session_id --name "$CONTAINER_NAME" "$IMAGE_NAME" || exit 1
else
